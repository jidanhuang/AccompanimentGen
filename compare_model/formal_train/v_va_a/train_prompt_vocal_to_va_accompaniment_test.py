...
def train(
    ...
    # whisper_model.eval()
    tf_logger = Logger('/data/huangrm/audioLM/musicgen_trainer/compare_model/formal_train/v2a_log/v_va_a')
    ...
    # model_path=
    model_path='/data/huangrm/audioLM/musicgen_trainer/compare_model/formal_train/v_va_a/saved_models/e_6/lm_29999.pt'
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "/data/huangrm/audioLM/musicgen_trainer/compare_model/formal_train/v_va_a/saved_models/e_6"
    ...
    for epoch in range(num_epochs):
        ...
        # for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            # else:
            #     lr_scheduler.step()   
            print(f"Epoch: {epoch+1}/{num_epochs}, Batch: {batch_idx+1}/{len(train_dataloader)}, Loss: {loss_va.item()}")
            ...
            tf_logger.scalar_summary('Train/Loss_va', loss_va.item(),current_step)
            tf_logger.scalar_summary('Train/Loss_a', loss_a.item(),current_step)
            ...
            if (current_step+1)%val_step==0 or lr==0.01:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, (audiopath, labels,vocalpath) in enumerate(tqdm(val_dataloader)):
                        ...
                        val_avg_loss_a+=loss_a.item()/len(val_dataloader)
                        val_avg_loss_va+=loss_va.item()/len(val_dataloader)
                            # val_avg_loss+=val_loss.item()/len(val_dataloader)
                        # assert count_nans(masked_logits) == 0
                            
                        print(f"Epoch: {epoch+1}/{num_epochs}, Batch: {batch_idx+1}/{len(train_dataloader)}, Val_Loss: {loss_va.item()}")
                    tf_logger.scalar_summary('Val/Loss_a', val_avg_loss_a,current_step)
                    tf_logger.scalar_summary('Val/Loss_va', val_avg_loss_va,current_step)
                    print(f'step={current_step},Val/Loss={val_avg_loss_va}')
