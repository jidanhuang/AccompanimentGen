...
def vocal_accompaniment_to_codes_melody(vocal_paths,accopaniment_paths,model:MusicGen,duration=6):
    codes,masks,melody_wavs=torch.tensor([]).to(torch.int64).cuda(),torch.tensor([]).cuda().to(torch.bool),[]
    for vocal_path,accopaniment_path in zip(vocal_paths,accopaniment_paths):
        ...
        melody_wavs=melody_wavs+get_melody_wavs(vocal_path,model,start_sample,duration)
    assert masks.shape[-1]==2*duration*model.frame_rate+4
    return codes,masks,melody_wavs
...
def train(
    ...
    tf_logger = Logger('/data/huangrm/audioLM/musicgen_trainer/compare_model/formal_train/v2a_log/vocal_melody_e_6')
    ...
    state_dict = torch.load('/data/huangrm/audioLM/musicgen_trainer/compare_model/formal_train/vocal_melody/saved_model/e_6/lm_13999.pt')
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "/data/huangrm/audioLM/musicgen_trainer/compare_model/formal_train/vocal_melody/saved_model/e_6"
    ...
    for epoch in range(num_epochs):
        ...
        # for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            # audio
            codes,pad_mask,melody_wavs  =vocal_accompaniment_to_codes_melody(vocalpath,audiopath,model)   
            attributes, _ = model._prepare_tokens_and_attributes(text, None,melody_wavs=melody_wavs)
