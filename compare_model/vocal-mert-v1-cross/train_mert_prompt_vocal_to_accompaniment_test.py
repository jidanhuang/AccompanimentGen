...
def va2wav_trim(audio_path,audio_path2, model: MusicGen, duration: int ):
    ...
    return wav,wav2,start_sample,end_sample
...
def vocal_accompaniment_to_codes(vocal_paths,accopaniment_paths,model:MusicGen,duration=6):
    ...
    for vocal_path,accopaniment_path in zip(vocal_paths,accopaniment_paths):
        vocal,accopaniment,start_sample,end_sample=va2wav_trim(vocal_path,accopaniment_path, model, duration)
    ...
    return codes,masks,start_samples,end_samples
...
def train(
    ...
    # whisper_model.eval()
    tf_logger = Logger('/data/huangrm/audioLM/musicgen_trainer/compare_model/v2a_log/vocal_mert-v1-cross_e_6_no_pos/')
    ...
    model = MusicGen.get_pretrained_mert(model_id)
    # model2 = MusicGen.get_pretrained(model_id)
    # model3 = MusicGen.get_pretrained(model_id)
    # model_path=
    state_dict = torch.load('/data/huangrm/audioLM/musicgen_trainer/compare_model/models_test/vocal-mert-v1-cross_e_6/lm_51999.pt')
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=True,num_workers=5,drop_last=True)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False,num_workers=5,drop_last=True)
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "/data/huangrm/audioLM/musicgen_trainer/compare_model/models_test/vocal_mert-v1-cross_e_6_no_pos/"
    ...
    for epoch in range(num_epochs):
        ...
        # for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            # audio
            codes,pad_mask,start_samples,end_samples  =vocal_accompaniment_to_codes(vocalpath,audiopath,model)   
            
            #text
            text =[ load_txt(label).strip() for label in labels]
            conditions, _ = model._prepare_tokens_and_attributes_audio(vocalpath, start_samples,end_samples,None)
            #训练无条件生成和有条件生成
            # null_conditions = ClassifierFreeGuidanceDropout(p=1.0)(conditions)
            # conditions = conditions + null_conditions
            #只训练有条件生成
            # condition_tensors=get_emb(text,tokenizer,whisper_model,model)
            tokenized = model.lm.condition_provider.tokenize_mert(conditions)#token id
            condition_tensors = model.lm.condition_provider(tokenized)#emb
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                # )
                lm_output = model.lm.compute_predictions_mert(
                    codes=codes,
                    conditions=[],
                    condition_tensors=condition_tensors
                )
