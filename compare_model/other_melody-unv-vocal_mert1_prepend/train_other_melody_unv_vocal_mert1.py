...
class TestAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return audio, label
def get_melody_wavs(melody_path,model:MusicGen,start_id,duration):
    melody, sr = torchaudio.load(melody_path)
    if melody.shape[-1]/sr>duration:
        melody=melody[:,start_id:start_id+int(duration*sr)]
    melody=melody[None]
    melody_wavs = [
        convert_audio(wav, sr, model.sample_rate, model.audio_channels)#sr->32000,1channel
...
def accompaniment_to_codes_melody(vocal_paths,accopaniment_paths,model:MusicGen,duration=6):
    codes,masks,melody_wavs,mert1_wavs=torch.tensor([]).to(torch.int64).cuda(),torch.tensor([]).cuda().to(torch.bool),[],[]
    for vocal_path,accopaniment_path in zip(vocal_paths,accopaniment_paths):
        ...
        # vocal_mask[...]=False
        accopaniment,start_sample=audio2wav_trim_2(accopaniment_path, model, duration)
        accopaniment=wav_to_codes(accopaniment.cuda(),model)
        accopaniment_mask=torch.ones_like(accopaniment, dtype=torch.bool)
        accopaniment_mask[...]=True
        #pad_mask
        b,K,T=accopaniment.shape
        pad_shape=[b,K,1]
        ...
        end_pad_token=pad_token.repeat(1,1,duration*model.frame_rate-accopaniment.shape[-1]).cuda()#300-229
        # mid_mask_token=mask_token.repeat(1,1,vocal.shape[-2]+duration*model.frame_rate-vocal.shape[-1]).cuda()
        end_mask_token=mask_token.repeat(1,1,duration*model.frame_rate-accopaniment.shape[-1]).cuda()#300-2*len

        #cat mask
        code=torch.cat([accopaniment,end_pad_token],dim=-1)
        mask=torch.cat([accopaniment_mask,end_mask_token],dim=-1)
        masks=torch.cat([masks,mask],dim=0)
        codes=torch.cat([codes,code],dim=0)
        melody_wavs=melody_wavs+get_melody_wavs(accopaniment_path,model,start_sample,duration)
        mert1_wavs=mert1_wavs+get_melody_wavs(vocal_path,model,start_sample,duration)
    assert mert1_wavs[0].shape==melody_wavs[0].shape
    assert masks.shape[-1]==duration*model.frame_rate
    return codes,masks,melody_wavs,mert1_wavs

def vocal_accompaniment_to_codes_melody(vocal_paths,accopaniment_paths,model:MusicGen,duration=6):
    codes,masks,melody_wavs,mert1_wavs=torch.tensor([]).to(torch.int64).cuda(),torch.tensor([]).cuda().to(torch.bool),[],[]
    for vocal_path,accopaniment_path in zip(vocal_paths,accopaniment_paths):
        # vocal,accopaniment=va2wav_trim(vocal_path,accopaniment_path, model, duration)
        vocal,start_sample=audio2wav_trim_2(vocal_path, model, duration)
...
def train(
    ...
    # whisper_model.eval()ps -p 141870 -o user

    tf_logger = Logger('/data/huangrm/audioLM/musicgen_trainer/compare_model/v2a_log/other-piano_melody_unv_vocal_mert1_e_6')
    # tf_logger = Logger(os.path.join(sys.path[0]+'/opencpop_log/'))
    if use_wandb is True:
        import wandb
        run = wandb.init(project='audiocraft')
    model = MusicGen.get_pretrained_v2a_melody_mert1_prepend(model_id)
    # model = MusicGen.get_pretrained_v2a_melody(model_id)
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "/data/huangrm/audioLM/musicgen_trainer/compare_model/models_test/v2a/other-piano_melody_unv_vocal_mert1_e_6/"
    ...
    for epoch in range(num_epochs):
        ...
        # for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            text =[ None for label in labels]
            # audio
            codes,pad_mask,melody_wavs,mert1_wavs  =accompaniment_to_codes_melody(vocalpath,audiopath,model)   
            # attributes, _ = model._prepare_tokens_and_attributes_audio(text, None,melody_wavs=melody_wavs)
            attributes, _ = model._prepare_tokens_and_attributes_melody_mert1(text, None,melody_wavs=melody_wavs,wav_for_mert=mert1_wavs)
            ...
            tokenized = model.lm.condition_provider.tokenize_melody_mert1(conditions)
