...
def train(
    ...
    setup_seed(seed_i)
    off_step=359999
    tf_logger = Logger('/data/huangrm/audioLM/musicgen_trainer/compare_model/v2a_log/v2a_beatnet_addnoise_e_6')
    if use_wandb is True:
        import wandb
        run = wandb.init(project='audiocraft')
    save_path = "/data/huangrm/audioLM/musicgen_trainer/compare_model/models_test/v2a_beatnet_addnoise_e_6"
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=4, shuffle=True,num_workers=5)
    val_dataset = Accompaniment(dataset_path,'val')
    val_dataloader = DataLoader(val_dataset, batch_size=4, shuffle=False,num_workers=5)
    learning_rate = lr
