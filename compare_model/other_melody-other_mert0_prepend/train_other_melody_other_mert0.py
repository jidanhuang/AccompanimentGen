...
def train(
    ...
    tf_logger = Logger('/data/huangrm/audioLM/musicgen_trainer/compare_model/v2a_log/other-piano_melody_other_mert0_e_6')
    # tf_logger = Logger(os.path.join(sys.path[0]+'/opencpop_log/'))
    if use_wandb is True:
        import wandb
        run = wandb.init(project='audiocraft')
    model = MusicGen.get_pretrained_v2a_melody_mert0_prepend(model_id)
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "/data/huangrm/audioLM/musicgen_trainer/compare_model/models_test/v2a/other-piano_melody_other_mert0_e_6/"
