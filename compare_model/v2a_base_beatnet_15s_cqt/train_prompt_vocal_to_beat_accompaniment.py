...
def wav_to_codes(wav,model:MusicGen):
    ...
    return codes
def vocal_accompaniment_to_codes(vocal_paths,accopaniment_paths,model:MusicGen,duration=15):
...
class Accompaniment(Dataset):
    def __init__(self, 
        ...
        self.data_map = []
        random.seed(42)
        dir_map = os.listdir(os.path.join(data_dir,f'{datatype}_cutlrc_15s'))
        ...
        for d in dir_map:
            ...
            if ext == '.txt':
                if os.path.exists(os.path.join(data_dir+'/cutmp3_15s',name , 'accompaniment.mp3')) and os.path.exists(os.path.join(data_dir+'/cutmp3_15s',name , 'vocals.mp3')) :#and os.path.exists( os.path.join(va_dir+'/cutmp3_15s',f'{name}.mp3')):
                    self.data_map.append({
                        "vocal": os.path.join(data_dir+'/cutmp3_15s',name , 'vocals.mp3'),
                        "audio": os.path.join(data_dir+'/cutmp3_15s',name , 'accompaniment.mp3'),
                        "va": os.path.join(va_dir+'/cutmp3_15s',f'{name}.mp3'),
                        "label": os.path.join(data_dir,f'{datatype}_cutlrc_15s', name + '.txt')
...
def train(
    ...
    setup_seed(seed_i)
    off_step=0
    tf_logger = Logger(logpath)
    if use_wandb is True:
        import wandb
        run = wandb.init(project='audiocraft')
    model = MusicGen.get_pretrained_beatnet_cqtmert(model_id)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (audiopath, labels,vocalpath,va) in enumerate(tqdm(train_dataloader)):
            ...
            cos_scheduler.step((epoch*len(train_dataloader)+batch_idx)/val_step) #t0为val_stepstep
            optimizer.zero_grad()
            #text
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                lm_output, beat_logits,beat,cqt_pred,cqt_target= model.lm.compute_predictions_beatnet_cqtmert(
                    codes=codes,
                    conditions=[],
                ...
                mask = mask.cuda()
                end_id=[sum(pad_m[0])+4+15*50 for pad_m in pad_mask]
                ...
                loss_cqt=criterion_cqt(cqt_pred[mask[:,0,:],:],cqt_target[mask[:,0,:],:])
                loss=0.5*loss1+0.25*loss_beat+0.25*loss_cqt
            loss.backward()
            optimizer.step()
            torch.nn.utils.clip_grad_norm_(model.lm.parameters(),0.5)
            print(f"Epoch: {epoch+1}/{num_epochs}, Batch: {batch_idx+1}/{len(train_dataloader)}, Loss: {loss1.item()}")
            ...
            tf_logger.scalar_summary('Train/Loss', loss1.item(),current_step)
            tf_logger.scalar_summary('Train/Loss_beat', loss_beat.item(),current_step)
            tf_logger.scalar_summary('Train/Loss_cqt', loss_cqt.item(),current_step)
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, (audiopath, labels,vocalpath,va) in enumerate(tqdm(val_dataloader)):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.float16):
                            ...
                            loss_beat = criterion(masked_beat_logits,masked_beat)
                            loss_cqt=criterion_cqt(cqt_pred[mask[:,0,:],:],cqt_target[mask[:,0,:],:])
                            val_loss=0.5*loss1+0.25*loss_beat+0.25*loss_cqt
                            
                            val_avg_loss+=loss1.item()/len(val_dataloader)
                            val_avg_loss_beat+=loss_beat.item()/len(val_dataloader)
                            val_avg_loss_cqt+=loss_cqt.item()/len(val_dataloader)
                    ...
                    tf_logger.scalar_summary('Val/Loss_cqt', val_avg_loss_cqt,current_step)
