...
model.lm.eval()
model_path='/data/huangrm/audioLM/musicgen_trainer/compare_model/models_test/v2a_beatnet_addnoise_e_6/lm_509999.pt'#355999,387999(最低loss 3.829),485999,
...
for lrc_file,wav_file in zip(lrc_files,wav_files):
    ...
    wav = model.generate_v2a_pad6s_beatnet(vocal_path,descriptions,addnoise=True)  # generates 3 samples.
    audio_write(f'/data/huangrm/audioLM/musicgen_trainer/compare_model/test/test_result/opencpop/a/beatnet/{wav_file}', wav[0].cpu(), model.sample_rate, strategy="loudness")#,loudness_compressor=True)
