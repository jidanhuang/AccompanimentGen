...
class StreamingTransformerLayer(nn.TransformerEncoderLayer):
    ...
    def __init__(self, d_model: int, num_heads: int, dim_feedforward: int = 2048, dropout: float = 0.1,
                 ...
                 kv_repeat: int = 1, norm: str = 'layer_norm', device=None, dtype=None, cross_input_norm=False,**kwargs):
        ...
        self.cross_input_norm=cross_input_norm
        if cross_attention:
            ...
            if cross_input_norm:
                self.norm_cross_text = nn.LayerNorm(d_model, eps=1e-5, **factory_kwargs)
    ...
    def forward(self, src: torch.Tensor, src_mask: tp.Optional[torch.Tensor] = None,  # type: ignore
        ...
        x = src
        if self.cross_input_norm:
            x = x + self.layer_scale_1(
                self._sa_block(self.norm1(x), src_mask, src_key_padding_mask))
            if cross_attention_src is not None:
                x = x + self.layer_scale_cross(
                    self._cross_attention_block(
                        self.norm_cross(x), self.norm_cross_text(cross_attention_src)))
    ...
    def forward1(self, src: torch.Tensor, src_mask: tp.Optional[torch.Tensor] = None,  # type: ignore
                src_key_padding_mask: tp.Optional[torch.Tensor] = None,
                cross_attention_src: tp.Optional[torch.Tensor] = None):
        if self.cross_attention is None:
            assert cross_attention_src is None
...
class StreamingTransformer(StreamingModule):
    ...
    def __init__(self, d_model: int, num_heads: int, num_layers: int, dim_feedforward: int = 2048,
        ...
        print(f'self_at_positional_scale={positional_scale}')
