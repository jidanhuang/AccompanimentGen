...
from models.t5pegasus.tokenizer import JieBaTokenizer#T5PegasusTokenizer
# from transformers.models.mt5.modeling_mt5 import T5ForConditionalGeneration
...
import spacy
from transformers import T5EncoderModel, T5Tokenizer,BertTokenizer ,Wav2Vec2FeatureExtractor,AutoModel # type: ignore
import torchaudio
import torch
...
@dataclass
class ConditioningAttributes_mert:
    wav_for_mert: tp.Dict[str, tp.Optional[Tensor]] = field(default_factory=dict)
    # length: Tensor
    # path: tp.List[tp.Optional[str]] = []
    ...
    @property
    def wav_for_mert_attributes(self):
        return self.wav_for_mert.keys()
    ...
    # @property
    # def start_t_attributes(self):
    #     return self.start_t.keys()

    # @property
    # def end_t_attributes(self):
    #     return self.end_t.keys()

    @property
    def attributes(self):
        return {"wav_for_mert": self.wav_for_mert_attributes, "wav": self.wav_attributes
                # "start_t": self.start_t_attributes, "end_t": self.end_t_attributes
                }

    def to_flat_dict(self):
        return {
            **{f"wav_for_mert.{k}": v for k, v in self.wav_for_mert.items()},
            **{f"wav.{k}": v for k, v in self.wav.items()},
            # **{f"start_t.{k}": v for k, v in self.start_t.items()},
            # **{f"end_t.{k}": v for k, v in self.end_t.items()},
        }
    ...
    def from_flat_dict(cls, x):
        ...
        return out
@dataclass
class ConditioningAttributes_mert1:
    wav_for_mert: tp.Dict[str, WavCondition] = field(default_factory=dict)
    wav: tp.Dict[str, WavCondition] = field(default_factory=dict)
...
class MertConditioner0(BaseConditioner):
...
class MertConditioner(TextConditioner):
    ...
    MODELS = ["t5-small", "t5-base", "t5-large", "t5-3b", "t5-11b",
              ...
              "google/flan-t5-xl", "google/flan-t5-xxl","models/t5-pegasus","models/t5-base-Chinese","/data/huangrm/audioLM/bark_trainer_hubert/bark-gui/models/MERT_v1_330M"]
    MODELS_DIMS = {
        "t5-small": 512,
        "t5-base": 768,
        "t5-large": 1024,
        ...
        "models/t5-pegasus":768,
        "models/t5-base-Chinese":768,
        "/data/huangrm/audioLM/bark_trainer_hubert/bark-gui/models/MERT_v1_330M":1024
    }
    ...
    def __init__(self, name: str, output_dim: int, finetune: bool, device: str,
        ...
        with warnings.catch_warnings():
            ...
            try:
                if name=='t5-base':
                    name='models/t5-base'
                if name=='models/t5-pegasus':
                    name="/data/huangrm/audioLM/musicgen_trainer/models/t5pegasus"
                    self.t5_tokenizer = JieBaTokenizer.from_pretrained(name)
                    # self.t5_tokenizer = BertTokenizer.from_pretrained(name)
                    t5 = T5EncoderModel.from_pretrained(name).train(mode=finetune)
                elif name=="/data/huangrm/audioLM/bark_trainer_hubert/bark-gui/models/MERT_v1_330M":
        ...
        if finetune:
            ...
            self.__dict__["mert"] = mert.to(device)
    ...
    def forward(self, inputs: WavCondition) -> ConditionType:
        ...
        wav, lengths, path = inputs
        device=wav.device
        wav=[w.squeeze().cpu().numpy() for w in wav]
        resample_rate = self.processor.sampling_rate#24000模型需要24000
        inputs = self.processor(wav, sampling_rate=resample_rate, return_tensors="pt",padding=True)#torch.Size([140520]),5.855s
        for at in inputs:
            inputs[at]=inputs[at].to(device)
        with torch.no_grad():
            outputs = self.mert(**inputs, output_hidden_states=True)
            states=outputs.hidden_states[7]
        mask=torch.ones_like(states[:,:,0],dtype=torch.bool,device=device)
        for i in range(len(mask)):
            # for j in range(mask.shape[1]):
            #     mask[i,j,:]=inputs["attention_mask"][i,int(resample_rate*(j+1)/75)]
            j=int(np.floor(lengths[i].cpu()/resample_rate*75))
            mask[i,j:]=False
        return states, mask
        ...
        # self, inputs: tp.Dict[str, torch.Tensor] 
        if self.name=='models/t5-pegasus':
            inputs = dict(
...
class T5Conditioner(TextConditioner):
    ...
    MODELS = ["t5-small", "t5-base", "t5-large", "t5-3b", "t5-11b",
              "google/flan-t5-small", "google/flan-t5-base", "google/flan-t5-large",
              "google/flan-t5-xl", "google/flan-t5-xxl","models/t5-pegasus","models/t5-base-Chinese"]
    MODELS_DIMS = {
        "t5-small": 512,
        "t5-base": 768,
        "t5-large": 1024,
    ...
    def __init__(self, name: str, output_dim: int, finetune: bool, device: str,
        ...
        assert name in self.MODELS, f"unrecognized t5 model name (should in {self.MODELS})"
        super().__init__(self.MODELS_DIMS[name], output_dim)
        name="models/t5-pegasus"
        # name="t5-base"
        # name='models/t5-base-Chinese'
        finetune=False
        self.device = device
        self.name = name
        self.finetune = finetune
...
class WaveformConditioner(BaseConditioner):
    ...
    def forward(self, inputs: WavCondition) -> ConditionType:
        ...
        wav, lengths, path = inputs
        with torch.no_grad():
            embeds = self._get_wav_embedding(wav)#B,235,12，大约为7.8HZ,pad或者trim到235对应为30s
        embeds = embeds.to(self.output_proj.weight)
        embeds = self.output_proj(embeds)#B,235,12->B,235,1536

        if lengths is not None:
            lengths = lengths / self._downsampling_factor()#和弦提取的采样率缩小倍数
            mask = length_to_mask(lengths, max_len=embeds.shape[1]).int()  # type: ignore
            #:根据length获得pad的mask,  mask和embeds前维度相同B,235
        else:
            mask = torch.ones_like(embeds)
        embeds = (embeds * mask.unsqueeze(2).to(self.device))#进行一个mask
        if embeds.shape[1]!=235:
            embeds=torch.nn.functional.pad(embeds,(0,0,0,235-embeds.shape[1]),value=0)
            mask=torch.nn.functional.pad(mask,(0,235-mask.shape[1]),value=False)
        return embeds, mask#B,235,1536;B,235

class Mert1Conditioner(BaseConditioner):
    """T5-based TextConditioner.
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                if name=='models/t5-pegasus':
                    ...
                    self.resample_rate=self.processor.sampling_rate
                    mert = AutoModel.from_pretrained(name, trust_remote_code=True)
                    self.resampler = torchaudio.transforms.Resample(32000, self.resample_rate)
                else:
                    self.t5_tokenizer = T5Tokenizer.from_pretrained(name)
                    t5 = T5EncoderModel.from_pretrained(name).train(mode=finetune)
            finally:
    ...
    def forward(self, inputs: WavCondition) -> ConditionType:
        ...
        wav, lengths, path = inputs
        device=wav.device
        wav=[self.resampler(w.squeeze()).cpu().numpy() for w in wav]
        resample_rate = self.processor.sampling_rate#24000模型需要24000
        inputs = self.processor(wav, sampling_rate=resample_rate, return_tensors="pt",padding=True)#torch.Size([140520]),5.855s
        for at in inputs:
            inputs[at]=inputs[at].to(device)
        ...
        for i in range(len(mask)):
            ...
            j=int(np.floor(lengths[i].cpu()/resample_rate*75))
            mask[i,j:]=False
        embeds = self.output_proj(states.to(self.output_proj.weight))
        embeds = (embeds * mask.unsqueeze(-1))
        if embeds.shape[1]!=450:
            embeds=torch.nn.functional.pad(embeds,(0,0,0,450-embeds.shape[1]),value=0)
            mask=torch.nn.functional.pad(mask,(0,450-mask.shape[1]),value=False)
...
class ChromaStemConditioner(WaveformConditioner):
    ...
    def _get_filtered_wav(self, wav):
        ...
        with self.autocast:
            ...
            stems = stems[:, self.stem_idx]  # extract stem，提取vocal和other
            stems = stems.sum(1)  # merge extracted stems  
            stems = stems.mean(1, keepdim=True)  # mono #两个音轨平均
    ...
    def _get_wav_embedding(self, wav):
        ...
        stems = self._get_filtered_wav(wav)#过滤vocal,drum,bass
        chroma = self.chroma(stems)#8hz? emb_dim=12

        if self.match_len_on_eval:
            b, t, c = chroma.shape#6,28,12
            if t > self.chroma_len:
                chroma = chroma[:, :self.chroma_len]
                logger.debug(f'chroma was truncated! ({t} -> {chroma.shape[1]})')
            elif t < self.chroma_len:#如果t<235,则重复取前235个
...
class ChromaExtractor(nn.Module):
    ...
    def forward(self, wav):
        with self.autocast:
            ...
            spec = self.spec(wav).squeeze(1)#hop_length=4096,sr=32000hz;torch.Size([6, 8193, 28]),nfft=window_Len
            raw_chroma = torch.einsum("cf,...ft->...ct", self.fbanks, spec)#torch.Size([6, 12, 28])
...
class ConditioningProvider(nn.Module):
    ...
    @property
    def mert_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(v, MertConditioner)]
    @property
    def mert1_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(v, Mert1Conditioner)]

    @property
    def has_wav_condition(self):
        return len(self.wav_conditions) > 0

    def tokenize_mert(self, inputs: tp.List[ConditioningAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        text = self._collate_wavs_for_mert(inputs)
    ...
    def tokenize_melody_mert1(self, inputs: tp.List[ConditioningAttributes]) -> tp.Dict[str, tp.Any]:
        """Match attributes/wavs with existing conditioners in self, and compute tokenize them accordingly.
        This should be called before starting any real GPU work to avoid synchronization points.
        This will return a dict matching conditioner names to their arbitrary tokenized representations.
        ...
        Args:
            inputs (list[ConditioningAttribres]): List of ConditioningAttributes objects containing
                text and wav conditions.
        """
        assert all([type(x) == ConditioningAttributes_mert1 for x in inputs]), \
            "got unexpected types input for conditioner! should be tp.List[ConditioningAttributes]" \
            f" but types were {set([type(x) for x in inputs])}"

        output = {}
        wavs = self._collate_wavs(inputs)
        wavs_for_mert1 = self._collate_wavs_for_mert1(inputs)


        for attribute, batch in chain( wavs.items(),wavs_for_mert1.items()):
            output[attribute] = self.conditioners[attribute].tokenize(batch)
        return output
    ...
    def forward(self, tokenized: tp.Dict[str, tp.Any]) -> tp.Dict[str, ConditionType]:
        ...
        return output
    def forward_mert(self,vocalpath, start_samples,end_samples) -> tp.Dict[str, ConditionType]:
        """Compute pairs of `(embedding, mask)` using the configured conditioners
        and the tokenized representations. The output is for example:
    ...
    def _collate_wavs_for_mert1(self, samples: tp.List[ConditioningAttributes]):
        ...
        *Note*: by the time the samples reach this function, each sample should have some waveform
        ...
        for sample in samples:
            for attribute in self.mert1_conditions:
                wav, length, path = sample.wav_for_mert[attribute]
        ...
        for attribute in self.mert1_conditions:
            ...
            out[attribute] = WavCondition(stacked_wav.unsqueeze(1),
                                          torch.cat(lens[attribute]), paths[attribute])  # type: ignore

        return out

    def _collate_wavs_for_mert(self, samples: tp.List[ConditioningAttributes]):
        """Generate a dict where the keys are attributes by which we fetch similar wavs,
        and the values are Tensors of wavs according to said attribtues.
        ...
        *Note*: by the time the samples reach this function, each sample should have some waveform
        ...
        for sample in samples:
            for attribute in self.mert_conditions:
                wav, length, path = sample.wav_for_mert[attribute]
                wavs[attribute].append(wav.flatten())
                lens[attribute].append(length)
                paths[attribute].append(path)
...
class ConditionFuser_mert(StreamingModule):
