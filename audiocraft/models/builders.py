...
def get_lm_model_5stems(cfg: omegaconf.DictConfig) -> LMModel:
...
def get_lm_model_mert(cfg: omegaconf.DictConfig) -> LMModel:
    """Instantiate a transformer LM.
    """
    if cfg.lm_model == 'transformer_lm':
        kwargs = dict_from_config(getattr(cfg, 'transformer_lm'))
        ...
        cls_free_guidance = dict_from_config(getattr(cfg, 'classifier_free_guidance'))
        cfg_prob, cfg_coef = cls_free_guidance["training_dropout"], cls_free_guidance["inference_coef"]
        fuser = get_condition_fuser_mert(cfg)
        condition_provider = get_conditioner_provider(kwargs["dim"], cfg).to(cfg.device)
        if len(fuser.fuse2cond['cross']) > 0:  # enforce cross-att programatically
            kwargs['cross_attention'] = True
        if codebooks_pattern_cfg.modeling is None:
...
def get_lm_model_beatnet(cfg: omegaconf.DictConfig) -> LMModel:
    """Instantiate a transformer LM.
    """
    if cfg.lm_model == 'transformer_lm':
        kwargs = dict_from_config(getattr(cfg, 'transformer_lm'))
        ...
        raise KeyError(f'Unexpected LM model {cfg.lm_model}')
def get_lm_model_beatnet_cqtmert(cfg: omegaconf.DictConfig) -> LMModel:
    """Instantiate a transformer LM.
    """
    if cfg.lm_model == 'transformer_lm':
        kwargs = dict_from_config(getattr(cfg, 'transformer_lm'))
        ...
        return LMModel(
            ...
            dtype=getattr(torch, cfg.dtype),
            device=cfg.device,
            mode='beatnet_cqtmert',
            **kwargs
        ).to(cfg.device)
    else:
        raise KeyError(f'Unexpected LM model {cfg.lm_model}')

def get_lm_model_melody_mert1_prepend(cfg: omegaconf.DictConfig) -> LMModel:
    """Instantiate a transformer LM.
    """
    if cfg.lm_model == 'transformer_lm':
        kwargs = dict_from_config(getattr(cfg, 'transformer_lm'))
...
def get_lm_model_melody(cfg: omegaconf.DictConfig) -> LMModel:
    """Instantiate a transformer LM.
    """
    if cfg.lm_model == 'transformer_lm':
        kwargs = dict_from_config(getattr(cfg, 'transformer_lm'))
...
def get_lm_model_v2a(cfg: omegaconf.DictConfig) -> LMModel:
    """Instantiate a transformer LM.
    """
    if cfg.lm_model == 'transformer_lm':
        kwargs = dict_from_config(getattr(cfg, 'transformer_lm'))
...
def get_conditioner_provider(output_dim: int, cfg: omegaconf.DictConfig) -> ConditioningProvider:
    ...
    for cond, cond_cfg in cfg.items():
        ...
        if model_type == "t5":
            ...
            conditioners[str(cond)] = MertConditioner(output_dim=output_dim, device=device, **model_args)
        elif model_type == "mert1":
            conditioners[str(cond)] = Mert1Conditioner(output_dim=output_dim, device=device, **model_args)
        else:
...
def get_condition_fuser_mert(cfg: omegaconf.DictConfig) -> ConditionFuser:
    ...
    fuse2cond['cross']=['wav_for_mert']
    fuser = ConditionFuser_mert(fuse2cond=fuse2cond, **kwargs)
    return fuser
