...
device='cuda'
model = MusicGen.get_pretrained_5stems('small',device=device)
...
# model_path='models/v2a_prompt/promt_304/5e_6/lm_27999.pt'
model_path='models/v2a_prompt/promt_304_5stems/5e_6/lm_13999.pt'
...
for lrc_file,wav_file in zip(lrc_files,wav_files):
    ...
    wav = model.generate_v2a_pad6s_5stems(vocal_path,descriptions)  # generates 3 samples.
    for i,onewav in enumerate(wav):
        wav_name=wav_file.replace('.wav','')
        ac_type=['drums','bass','piano','other']
        audio_write(f'test/opencpop/a/{wav_name}/{ac_type[i]}.wav', onewav[0].cpu(), model.sample_rate, strategy="loudness", loudness_compressor=True)
