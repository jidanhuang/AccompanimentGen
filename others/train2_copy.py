...
def train(
    ...
    # whisper_model.eval()
    tf_logger = Logger(os.path.join(sys.path[0]+'/a_log/'))
    ...
    state_dict = torch.load('/data/huangrm/audioLM/musicgen_trainer/models/accompaniment/multit5/lm_61999.pt',map_location='cuda:0')
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=12, shuffle=True,num_workers=5,drop_last=True)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=12, shuffle=False,num_workers=5,drop_last=True)
    ...
    save_path = "models/2_conv/"+dataset_path.split('/')[-1]
