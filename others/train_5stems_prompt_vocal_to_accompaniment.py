...
class Accompaniment5Stems(Dataset):
    def __init__(self, 
                data_dir,datatype
                ):
        self.data_dir = data_dir
        ...
        for d in dir_map:
            name, ext = os.path.splitext(d)
            if ext == '.txt':
                if os.path.exists(os.path.join(data_dir+'/cutmp3_5stems',name , 'vocals.mp3')):
                    self.data_map.append({
                        "vocal": os.path.join(data_dir+'/cutmp3_5stems',name , 'vocals.mp3'),
                        "drums": os.path.join(data_dir+'/cutmp3_5stems',name , 'drums.mp3'),
                        "bass": os.path.join(data_dir+'/cutmp3_5stems',name , 'bass.mp3'),
                        "piano": os.path.join(data_dir+'/cutmp3_5stems',name , 'piano.mp3'),
                        "other": os.path.join(data_dir+'/cutmp3_5stems',name , 'other.mp3'),
                        "label": os.path.join(data_dir,f'{datatype}_cutlrc', name + '.txt')
                    })
                else:
                    pass
    ...
    def __getitem__(self, idx):
        ...
        return drums,bass,piano,other, label,vocal
...
def va2wav_trim(audio_path,audio_path2, model: MusicGen, duration: int ):
    ...
    return wav,wav2
def stems2wav_trim(audio_path,audio_path2,audio_path3,audio_path4,audio_path5, model: MusicGen, duration: int ):
    wav, sr = torchaudio.load(audio_path)
    # wav=wav.cpu()
    # wav=wav.to(torch.float16) 
    wav = torchaudio.functional.resample(wav, sr, model.sample_rate)
    ...
    assert wav2.shape[1] <= model.sample_rate * duration
    wav2 = wav2.unsqueeze(1)
    if wav2.shape!=wav.shape:
        print(audio_path2)
    assert wav2.shape==wav.shape
    
    wav3, sr = torchaudio.load(audio_path3)
    # wav3=wav3.cpu()
    # wav3=wav3.to(torch.float16) 
    wav3 = torchaudio.functional.resample(wav3, sr, model.sample_rate)
    wav3 = wav3.mean(dim=0, keepdim=True)
    #trim
    if wav3.shape[1] > model.sample_rate * duration:
        wav3 = wav3[:,start_sample :end_sample] 
        assert wav3.shape[1] == model.sample_rate * duration
    assert wav3.shape[0] == 1
    assert wav3.shape[1] <= model.sample_rate * duration
    wav3 = wav3.unsqueeze(1)
    assert wav3.shape==wav.shape
    
    wav4, sr = torchaudio.load(audio_path4)
    # wav4=wav4.cpu()
    # wav4=wav4.to(torch.float16) 
    wav4 = torchaudio.functional.resample(wav4, sr, model.sample_rate)
    wav4 = wav4.mean(dim=0, keepdim=True)
    #trim
    if wav4.shape[1] > model.sample_rate * duration:
        wav4 = wav4[:,start_sample :end_sample] 
        assert wav4.shape[1] == model.sample_rate * duration
    assert wav4.shape[0] == 1
    assert wav4.shape[1] <= model.sample_rate * duration
    wav4 = wav4.unsqueeze(1)
    assert wav4.shape==wav.shape

    wav5, sr = torchaudio.load(audio_path5)
    # wav5=wav5.cpu()
    # wav5=wav5.to(torch.float16) 
    wav5 = torchaudio.functional.resample(wav5, sr, model.sample_rate)
    wav5 = wav5.mean(dim=0, keepdim=True)
    #trim
    if wav5.shape[1] > model.sample_rate * duration:
        wav5 = wav5[:,start_sample :end_sample] 
        assert wav5.shape[1] == model.sample_rate * duration
    assert wav5.shape[0] == 1
    assert wav5.shape[1] <= model.sample_rate * duration
    wav5 = wav5.unsqueeze(1)
    assert wav5.shape==wav.shape


    return wav,wav2,wav3,wav4,wav5
...
def vocal_5stems_to_codes(vocal_paths,drums_paths,bass_paths,piano_paths,other_paths,model:MusicGen,duration=6):
    codes,masks=torch.tensor([]).to(torch.int64).cuda(),torch.tensor([]).cuda().to(torch.bool)
    for vocal_path,drums_path,bass_path,piano_path,other_path in zip(vocal_paths,drums_paths,bass_paths,piano_paths,other_paths):
        #vocal
        vocal,drums,bass,piano,other=stems2wav_trim(vocal_path,drums_path,bass_path,piano_path,other_path, model, duration)
        ...
        drums=wav_to_codes(drums.cuda(),model)
        drums_mask=torch.ones_like(vocal, dtype=torch.bool)
        drums_mask[...]=True
        #bass
        bass=wav_to_codes(bass.cuda(),model)
        bass_mask=torch.ones_like(vocal, dtype=torch.bool)
        bass_mask[...]=True
        #piano
        piano=wav_to_codes(piano.cuda(),model)
        piano_mask=torch.ones_like(vocal, dtype=torch.bool)
        piano_mask[...]=True
        #other
        other=wav_to_codes(other.cuda(),model)
        other_mask=torch.ones_like(vocal, dtype=torch.bool)
        other_mask[...]=True
        #pad_mask
        b,K,T=vocal.shape
        pad_shape=[b,K,1]
        ...
        # pad_token[...]=2048
        mask_token[...]=False
        pad_tokens,infer_tokens,mid_pad_tokens=[],[],[]
        
        for idx in range(4):
            infer_token=torch.zeros(pad_shape).to(torch.int64)#B,K,K
            infer_token[...]=2048+2*idx#2048,2050,2052,2054
            pad_token=torch.zeros(pad_shape).to(torch.int64)#B,K,K
            pad_token[...]=2048+2*idx+1#2049,2051,2053,2055
            mid_pad_token=pad_token.repeat(1,1,duration*model.frame_rate-vocal.shape[-1]).cuda()#300-229
            mid_pad_token=torch.cat([mid_pad_token,infer_token.repeat(1,1,vocal.shape[-2]).cuda()],dim=-1)#4+300-229
            
            infer_tokens.append(infer_token)
            pad_tokens.append(pad_token)
            mid_pad_tokens.append(mid_pad_token)
        ...
        #cat mask
        code=torch.cat([vocal,mid_pad_tokens[0],drums,mid_pad_tokens[1],bass,mid_pad_tokens[2],piano,mid_pad_tokens[3],other,end_pad_token],dim=-1)
        mask=torch.cat([vocal_mask,mid_mask_token,drums_mask,mid_mask_token,bass_mask,mid_mask_token,piano_mask,mid_mask_token,other_mask,end_mask_token],dim=-1)
        
        masks=torch.cat([masks,mask],dim=0)
        codes=torch.cat([codes,code],dim=0)
    assert masks.shape[-1]==(duration*model.frame_rate+4)*4+duration*model.frame_rate
    return codes,masks
...
def train(
    ...
    # whisper_model.eval()
    tf_logger = Logger(os.path.join(sys.path[0]+'/v2a_log_prompt/promt_304_5stems/5e_6'))
    # tf_logger = Logger(os.path.join(sys.path[0]+'/opencpop_log/'))
    if use_wandb is True:
        import wandb
        run = wandb.init(project='audiocraft')

    model = MusicGen.get_pretrained_5stems(model_id)
    ...
    model.lm.emb[0].weight.data[:2049, :] =state_dict['emb.0.weight']
    model.lm.emb[1].weight.data[:2049, :] =state_dict['emb.1.weight']
    model.lm.emb[2].weight.data[:2049, :] =state_dict['emb.2.weight']
    model.lm.emb[3].weight.data[:2049, :] =state_dict['emb.3.weight']
    state_dict.pop('emb.0.weight')
    state_dict.pop('emb.1.weight')
    state_dict.pop('emb.2.weight')
    state_dict.pop('emb.3.weight')
    ...
    train_dataset = Accompaniment5Stems(dataset_path,'train')
    ...
    val_dataset = Accompaniment5Stems(dataset_path,'val')
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "models/v2a_prompt/promt_304_5stems/5e_6"
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (drums,bass,piano,other, labels,vocalpath) in enumerate(tqdm(train_dataloader)):
        # for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            # audio
            codes,pad_mask  =vocal_5stems_to_codes(vocalpath,drums,bass,piano,other,model)   
            #train  
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                end_id=[int(sum(pad_m[0])/4) for pad_m in pad_mask]

                for jdx in range(4):    
                    end_id =[e+4+300 for e in end_id]       
                    for b_id in range(len(pad_mask)):
