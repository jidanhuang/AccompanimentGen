...
if TYPE_CHECKING:
    from .model import Whisper,Multimodal_Whisper,Multimodal_Whisper1,Multimodal_Whisper0
...
class PyTorchInference(Inference):
    def __init__(self, model: "Multimodal_Whisper", initial_token_length: int):
        self.model: "Multimodal_Whisper" = model
        self.initial_token_length = initial_token_length#4
        self.kv_cache = {}
        self.hooks = []
    def multimodal_logits0(self, tokens: Tensor, audio_features: Tensor,image_features:Tensor) -> Tensor:
        ...
        # if self.option
        fusion_features=self.model.image_audio_fusion(image_features, audio_features) 

        return self.model.decoder(tokens, fusion_features, kv_cache=self.kv_cache)#return logits torch.Size([5, 1, 51865])

    def multimodal_logits(self, model_type,tokens: Tensor, audio_features: Tensor,language_features:Optional[Tensor],image_features:Optional[Tensor],clip2_f,context) -> Tensor:
        if not self.kv_cache:
            self.kv_cache, self.hooks = self.model.install_kv_cache_hooks()
        ...
        # fusion_features=self.model.image_audio_fusion(image_features, audio_features) 
        with torch.no_grad():
            if model_type=='caption_context2_bertfusion':
                fusion_features,_=self.model.language_audio_fusion(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features, kv_cache=self.kv_cache),_
            elif model_type=='caption_context_bertfusion':
                fusion_features,_=self.model.language_audio_fusion(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features, kv_cache=self.kv_cache),_#return logits torch.Size([5, 1, 51865])
            elif model_type=='caption_context' or model_type=='dcaption_context2' :
                return self.model.decoder(tokens, audio_features, kv_cache=self.kv_cache),None#return logits torch.Size([5, 1, 51865])
            elif model_type=='bert_both_mid_decoder_fusion':
                fusion_features,_=self.model.language_audio_fusion(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features,language_features[:,:1,:].repeat_interleave(tokens.shape[1], dim=-2)),_
            elif model_type=='stack_bert_gfusion':
                fusion_features = audio_features#encoder输出，[16, 1500, 512];[batch_size=sample_nums,token_size,emb_size]
                for fusion in self.model.fusion_blocks:
                    fusion_features,_=fusion(language_features, fusion_features) 
                return self.model.decoder(tokens, fusion_features),_
            elif model_type=='parallel_mlp_fusion':
                fusion_features1,_=self.model.image_audio_fusion(image_features, audio_features) #8,1500,512
                fusion_features2,_=self.model.language_audio_fusion(language_features, audio_features) 
                fusion_features=torch.concat((audio_features,fusion_features1,fusion_features2),dim=-1)#8, 1500, 1024
                fusion_features=self.model.mlp(fusion_features)
                return self.model.decoder(tokens, fusion_features),_
            elif model_type=='bert_emb_clip_bert': 
                fusion_features=self.model.language_audio_fusion(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features,language_features[0],clip2_f)
            elif model_type=='bert_8head':
                fusion_features,_=self.model.language_audio_fusion(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features),_
            elif model_type=='parallel_fusion':
                fusion_features1,_=self.model.image_audio_fusion(image_features, audio_features) #8,1500,512
                fusion_features2,_=self.model.language_audio_fusion(language_features, audio_features) 
                fusion_features=torch.concat((audio_features,fusion_features1,fusion_features2),dim=-1)#8, 1500, 1024
                fusion_features=self.model.fusion_dense(fusion_features)
                return self.model.decoder(tokens, fusion_features),_
            elif model_type=='decode_l_at':
                fusion_features,_=self.model.language_audio_fusion(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features,language_features),_
            elif model_type=='decode_languagefusion':
                return self.model.decoder(tokens, audio_features,language_features, kv_cache=self.kv_cache)#return logits torch.Size([5, 1, 51865])
            elif model_type=='parallel_gatefusion':
                # fusion_features1=self.model.image_audio_fusion(image_features, audio_features) #8,1500,512
                # fusion_features2=self.model.language_audio_fusion(language_features, audio_features) 
                # fusion_features=torch.concat((fusion_features1,fusion_features2),dim=-1)
                # fusion_features=self.model.mlp(fusion_features)
                # image_features=self.model.image_dense(image_features)
                # fusion_features=torch.concat((image_features,language_features),dim=-2)#8, 1500, 1024
                # fusion_features=self.model.lv_audio_fusion(fusion_features, audio_features) 
                temp_layer = copy.deepcopy(self.model.image_language_fusion).to(language_features.dtype)
                language_features,_=temp_layer(image_features, language_features) 
                # fusion_features=torch.concat((language_features,language_features),dim=-1)#8, 1500, 1024
                temp_layer = copy.deepcopy(self.model.lv_audio_fusion).to(language_features.dtype)
                fusion_features,_=temp_layer(language_features, audio_features) 
                return self.model.decoder(tokens, fusion_features, kv_cache=self.kv_cache),_#return logits torch.Size([5, 1, 51865])
            else:
                fusion_features=audio_features
                if image_features is not None:
                    # if image_features.dtype==torch.float16:
                    temp_layer = copy.deepcopy(self.model.image_audio_fusion).to(audio_features.dtype)
                    fusion_features,att_score=temp_layer(image_features, fusion_features) 
                if language_features is not None:
                    # if language_features.dtype==torch.float16:
                    temp_layer = copy.deepcopy(self.model.language_audio_fusion).to(audio_features.dtype)
                    fusion_features,att_score=temp_layer(language_features, fusion_features) 
                return self.model.decoder(tokens, fusion_features, kv_cache=self.kv_cache),att_score#return logits torch.Size([5, 1, 51865])
...
class MultimodalDecodingTask:
    ...
    def _main_loop(self,model_type, language_features:Optional[Tensor],image_features:Optional[Tensor],audio_features: Tensor, tokens: Tensor,clip2_f,context):
        assert audio_features.shape[0] == tokens.shape[0]
        n_batch = tokens.shape[0]#best_of/beam_size 5
        sum_logprobs: Tensor = torch.zeros(n_batch, device=audio_features.device)#[5]
        no_speech_probs = [np.nan] * n_batch
        ...
        try:
            for i in range(self.sample_len):
                logits,att_score = self.inference.multimodal_logits(model_type,tokens, audio_features,language_features,image_features,clip2_f,context)#获得新词logits；torch.Size([5, 4, 51865])表示右移一格的得分
                #非第一个词：torch.Size([5, 4, 51865]) 5个seq的新tokne得分
                if i == 0 and self.tokenizer.no_speech is not None:  # save no_speech_probs
                    probs_at_sot = logits[:, self.sot_index].float().softmax(dim=-1)
                    no_speech_probs = probs_at_sot[:, self.tokenizer.no_speech].tolist()
                ...
                # 应用logit过滤器，例如抑制或应用惩罚；生成第一个时抑制空格，此外还抑制特殊token字符
                for logit_filter in self.logit_filters:
                    logit_filter.apply(logits, tokens)#50258,50360,50361,50362
                # expand the tokens tensor with the selected next tokens#返回五个候选的完整token
                tokens, completed = self.decoder.update(tokens, logits, sum_logprobs)#best_of和beam在此处不同；输入5个seq新词的logits和k*seq_len的token和5个候选的累计得分；返回torch.Size([5, 5])，
                #如果预测完了（eot）或者token长度超过n_ctx，停止生成
                if completed or tokens.shape[-1] > self.n_ctx:#tensor(True, device='cuda:0')
        ...
        #最后输出tokens[5,33][beam_size,seq_len of finaloutput]
        #sum_logprobs:[5]五个candidates的log_probs
        return tokens, sum_logprobs, no_speech_probs,att_score#torch.Size([5, 140])；torch.Size([5])

    @torch.no_grad()
    def run(self, model_type,mel: Tensor, image_features:Optional[Tensor],language_features:Optional[Tensor],clip2_f,context) -> List[DecodingResult]:
        self.decoder.reset()
        tokenizer: Tokenizer = self.tokenizer
        n_audio: int = mel.shape[0]#1
        #torch.Size([1, 1500, 512])
        audio_features: Tensor = self._get_audio_features(mel)  # encoder forward pass
        tokens: Tensor = torch.tensor([self.initial_tokens]).repeat(n_audio, 1)
        if model_type=='caption_context_bertfusion' or model_type=='caption_context2_bertfusion' or model_type=='caption_context' or model_type=='dcaption_context2'or model_type=='dcaption_context5':
            tokens=torch.cat((context,tokens.to(context.device)),dim=-1)
            self.sample_begin=context.shape[-1]+self.sample_begin
            self.inference.initial_token_length=context.shape[-1]+len(self.initial_tokens)
            self.logit_filters[0].sample_begin=self.sample_begin
        #torch.Size([1, 4])(n_audio,token_nums)
        ...
        # beam_size=5
        audio_features = audio_features.repeat_interleave(self.n_group, dim=0)  #torch.Size([5, 1500, 512])
        image_features = image_features.repeat_interleave(self.n_group, dim=0) if image_features is not None else None#torch.Size([5, 1500, 512])
        language_features = language_features.repeat_interleave(self.n_group, dim=0) if language_features is not None else None#torch.Size([5, 1500, 512])
        
        
        tokens = tokens.repeat_interleave(self.n_group, dim=0).to(audio_features.device)#[5,4]
        # call the main sampling loop
        tokens, sum_logprobs, no_speech_probs,att_score = self._main_loop(model_type,language_features,image_features,audio_features, tokens,clip2_f,context)

        # from visualize import visualize_grid_attention_v2
        # import numpy as np
        # import cv2
        # import os
        # img_path="data_narrative/images/open_images_test_localized_narratives/00090369f46e7b9e.jpg"
        # save_path="visualize"
        # import cv2
        # import numpy as np
        # import os
        ...
        # # Assuming that `fig_list` contains your list of figures
        # fig_list = []
        # for i in range(15):
            
        #     attention_mask = att_score[0, i*100, :].cpu().numpy().astype(np.float32).reshape([7, 7])
        #     fig = visualize_grid_attention_v2(i,img_path,
        #                                     save_path=save_path,
...
@torch.no_grad()
def multimodal_decode0(model: "Multimodal_Whisper0", mel: Tensor,image_features:Tensor, options: DecodingOptions = DecodingOptions()) -> Union[DecodingResult, List[DecodingResult]]:
    """
    Performs decoding of 30-second audio segment(s), provided as Mel spectrogram(s).
    ...
    if single:
        mel = mel.unsqueeze(0)
    #mel[1,80,3000],80bands,3000time
    result = MultimodalDecodingTask(model, options).run('clip',mel,image_features,None)
...
@torch.no_grad()
def multimodal_decode1(model: "Multimodal_Whisper1", mel: Tensor,image_features:Tensor, language_features:Tensor,options: DecodingOptions = DecodingOptions()) -> Union[DecodingResult, List[DecodingResult]]:
    """
    Performs decoding of 30-second audio segment(s), provided as Mel spectrogram(s).
    ...
    if single:
        mel = mel.unsqueeze(0)
    #mel[1,80,3000],80bands,3000time
    result = MultimodalDecodingTask(model, options).run(mel,image_features,language_features)
...
@torch.no_grad()
def multimodal_decode(model: "Multimodal_Whisper",model_type, mel: Tensor,image_features:Tensor, language_features:Tensor,options: DecodingOptions = DecodingOptions(),clip2_f:Optional[Tensor]=None,context=None) -> Union[DecodingResult, List[DecodingResult]]:
    """
    Performs decoding of 30-second audio segment(s), provided as Mel spectrogram(s).
    ...
    if single:
        mel = mel.unsqueeze(0)
    #mel[1,80,3000],80bands,3000time
    result = MultimodalDecodingTask(model, options).run(model_type,mel,image_features,language_features,clip2_f,context)
