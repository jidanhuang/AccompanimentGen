...
def audio2wav_trim(audio_path, model: MusicGen, duration: int = 6):
...
def wav_to_codes(wav,model:MusicGen):
    ...
    return codes
def vocal_accompaniment_to_codes(vocal_paths,accopaniment_paths,model:MusicGen,duration):
    codes,masks=torch.tensor([]),torch.tensor([])
    for vocal_path,accopaniment_path in zip(vocal_paths,accopaniment_paths):
        vocal=audio2wav_trim(vocal_path, model, duration)
        vocal=wav_to_codes(vocal,model)
        vocal_mask=torch.ones_like(vocal, dtype=torch.bool)
        vocal_mask[...]=False
        accopaniment=audio2wav_trim(accopaniment_path, model, duration)
        accopaniment=wav_to_codes(accopaniment,model)
        accopaniment_mask=torch.ones_like(vocal, dtype=torch.bool)
        accopaniment_mask[...]=True
        #pad_mask
        pad_shape=vocal.shape
        pad_shape[-1]=1
        pad_token=torch.zeros(pad_shape)#B,K,K
        mask_token=torch.zeros(pad_shape)#B,K,K
        pad_token[...]=2048
        mask_token[...]=False
        
        mid_pad_token=pad_token.repeat(vocal.shape[-2], -1)
        end_pad_token=pad_token.repeat(duration*50-vocal.shape[-1]*2, -1)
        mid_mask_token=mask_token.repeat(vocal.shape[-2], -1)
        end_mask_token=mask_token.repeat(duration*50-vocal.shape[-1]*2, -1)#300-2*len
        #cat mask
    ...
    assert masks.shape[-1]<=duration*50+4
    return codes,masks
...
def train(
    ...
    # whisper_model.eval()
    tf_logger = Logger(os.path.join(sys.path[0]+'/v2a_log2/'))
    # tf_logger = Logger(os.path.join(sys.path[0]+'/opencpop_log/'))
    if use_wandb is True:
        import wandb
        run = wandb.init(project='audiocraft')

    model = MusicGen.get_pretrained_v2a(model_id)

    # state_dict = torch.load(f"models/2/accompaniment/lm_113999.pt")
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=12, shuffle=True,num_workers=5)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=12, shuffle=False,num_workers=5)
    ...
    save_path = "models/v2a/"+dataset_path.split('/')[-1]
    ...
    for epoch in range(num_epochs):
        ...
        # for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                end_id=[2*sum(pad_m[0])+4 for pad_m in pad_mask]
                # tri_mask=mask[0][:,-3:]
            ...
            if (current_step+1)%val_step==0 or lr==0.01:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, (audiopath, labels,vocalpath) in enumerate(tqdm(val_dataloader)):
                        ...
                        for audio,label in zip(vocalpath, labels):
                            vocal,pad_mask = preprocess_audio(audio, model) #gen_code: torch.Size([1, 4, 1500]) returns tensor,B,K,T
                            vocals = torch.cat((vocals, vocal), dim=0)
                            # audio_mask = torch.cat((audio_mask, pad_mask), dim=0)
                        # audio_mask=audio_mask.cuda()
                        vocals=vocals.cuda()
                        vocals=wav_to_codes(vocals,model)
                        vocals.cuda()#.to(torch.float16) 
                        ...
                        # codes =audio#[2B,K,T]
                        with torch.autocast(device_type="cuda", dtype=torch.float16):
                            lm_output = model.lm.compute_predictions_v2a(
                                codes=codes,
                                conditions=[],
                                condition_tensors=condition_tensors
                            )
