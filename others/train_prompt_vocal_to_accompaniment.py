...
class Accompaniment(Dataset):
    def __init__(self, 
        ...
        for d in dir_map:
            name, ext = os.path.splitext(d)
            if ext == '.txt':
                if os.path.exists(os.path.join(data_dir+'/cutmp3',name , 'accompaniment.mp3')):
                    self.data_map.append({
...
def train(
    ...
    # whisper_model.eval()
    tf_logger = Logger(os.path.join(sys.path[0]+'/v2a_log_prompt/promt_304/addnoise_e_6_37999'))
    ...
    model.lm = model.lm.to(torch.float16) #important
    ...
    train_dataset = Accompaniment(dataset_path,'val')
...
# debugfs -R 'stat <56755622>' /dev/nvme0n1p2
    save_path = "models/v2a_prompt/promt_304/addnoise_e_6_37999"
